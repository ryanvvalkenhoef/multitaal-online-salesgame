class GameController {
  ...
  checkIfQueueNotEmptyAndSendAnswer = (socket, playerId) => {
    ...
    if (questionQueueLength !== 0) {
      ...
      this.#socketManager.emitBackToClient(socket, "player_has_been_reviewed", {
        playerId: playerId,
  ...
  updateGameState = (socket) => {
    // Sends newest game state to the client
  ...
  startRound = (socket) => {
    ...
    for (let i = 0; i < playerNames.length; i++) {
      ...
      this.#socketManager.emitToSpecificSocket(
        socketId,
        "players_turn",
        strategy,
      );
