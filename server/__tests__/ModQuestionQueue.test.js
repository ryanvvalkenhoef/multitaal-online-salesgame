...
describe("ModQuestionQueue", () => {
  ...
  test("should get all playeranswers from the queue by player", () => {
    ...
    const retrievedQuestion1 = queueInstance.getQuestionFromQueue(
      socket,
      playerId1,
    );
    ...
    const retrievedQuestion2 = queueInstance.getQuestionFromQueue(
      socket,
      playerId1,
    );
    expect(retrievedQuestion2).toEqual(questionData2);

    expect(queueInstance.getQuestionFromQueue(socket, playerId2)).toEqual(
      questionData3,
    );
