...
describe("GameManager", () => {
  ...
  test("sendAnswerToModerator should call emitToMod with correct arguments", () => {
    ...
    //THEN
    expect(mockSocketManager.emitToMod).toHaveBeenCalledWith(
      mockSocket,
  ...
  test("checkIfQueueNotEmptyAndSendAnswer should handle non-empty queue", () => {
    ...
    //THEN
    expect(mockModQuestionQueue.getQuestionQueueLength).toHaveBeenCalledWith(
      mockSocket,
      playerId,
    );
    expect(mockModQuestionQueue.getQuestionFromQueue).toHaveBeenCalledWith(
      mockSocket,
      playerId,
    );
    expect(mockSocketManager.emitToMod).toHaveBeenCalledWith(
  ...
  test("checkIfQueueNotEmptyAndSendAnswer should handle empty queue", () => {
    ...
    expect(mockUserLogger.setHasBeenReviewed).toHaveBeenCalledWith(
      playerId,
      true,
    );
    expect(mockSocketManager.emitBackToClient).toHaveBeenCalledWith(
      mockSocket,
      "player_has_been_reviewed",
      { playerId, hasBeenReviewed: true },
    );
  });
});
