  ...
  io.on("connection", (socket) => {
    ...
    const socketHandlers = {
      ...
      disconnect: (reason) => {
        ...
        socket
          .to(userLogger("getRoom", socket.id))
          .emit("delete_user", "deleting");
      ...
      send_question_request: async (data) => {
        ...
        if (availableColors.includes(data.questionColor)) {
          const receiver = userLogger("getReceiver", socket.id, {
            color: data.questionColor,
            room: room,
          });
          io.to(receiver).emit("receive_question", {
      ...
      send_points: (data) => {
        ...
        const points = userLogger(
          "updatePoints",
          socket.id,
          parseInt(oldPoints + data.points)
        );
      },
      ...
      get_playerstrategy: (data) => {
        ...
        socket.emit("register_currentplayer", {
          strategy: strategy,
          color: color,
        });
      },
      ...
      question_reviewed: () => {
        ...
        if (questionQueueLength > 0) {
          ...
          gameMethods.send_answer_to_moderator(socket, question);
        }
        // als queue leeg is maar niet alle speler geanwoord hebben hoeft er niks te gebeuren
      },
    };
